...
[NOTE]
====
Note that the above only works when making the request with the access token. Inspect both tokens in the dev UI and you will find that
only the access token contains the list of roles. To tell Keycloak to include this information in the ID token as well, add the
...
[Keycloak server administration guide](https://www.keycloak.org/docs/latest/server_admin/#protocol).
===
